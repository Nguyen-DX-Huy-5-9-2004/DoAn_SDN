...
# =====================================================================
NUM_FEATURES = 13              # 13 trường dữ liệu gốc từ batPack
NUM_FEATURES_DIFF = 13         # 13 đặc trưng biến thiên (Differential)
...
SWITCH_CONFIG = {
    "ingress_switch": "s6",      # Gateway - Chặng 1 (Ngăn chặn sớm nhất)
    "secondary_switch": "s5",    # Backend - Chặng 2 (Bảo vệ WEB_SERVER)
    "default_drop_priority": 1000,  # Độ ưu tiên cao (ghi đè các luật thông thường)
    "timeout_sec": 600           # Giữ lệnh DROP trong 10 phút
}
...
# Danh sách đặc trưng BIẾN THIÊN (Differential - tính sự thay đổi từ flow trước)
...
# =====================================================================
# 2. QUẢN LÝ MÔ HÌNH (MODEL MANAGER - NÂNG CẤP)
# =====================================================================
...
class Anomaly_Autoencoder_Contrastive(nn.Module):
    ...
    def __init__(self, input_dim=NUM_FEATURES_TOTAL * SEQ_LEN):
        ...
        # Decoder: Giải nén từ latent space
...
class DDos_ParallelFusion_CNN_GRU_Attention(nn.Module):
    ...
    def __init__(self, input_dim=NUM_FEATURES_TOTAL):
        ...
        self.gru = nn.GRU(
            input_size=input_dim,  # Từ CNN output
            hidden_size=128, 
            num_layers=2, 
            batch_first=True, 
            dropout=0.3, 
